...
msra_valid = scipy.io.loadmat(msra_mat_dir)['msra_valid'] 
...
for sub_idx in range(len(subject_names)):
    ...
    for ges_idx in range(len(gesture_names)):
        ...
        for frm_idx in range(len(depth_files)):
            ...
            # ax.scatter(hand_3d[:, 0], hand_3d[:, 1], hand_3d[:, 2])
            # ax.set_xlabel('X Label')
            # ax.set_ylabel('Y Label')
            # ax.set_zlabel('Z Label')
            # ax = fig.add_subplot(212)
            # ax.imshow(hand_depth)
            # plt.show()
        ...
        #     # compute surface normals
            ...
            for k in range(SAMPLE_NUM):
                ...
                angle = np.arctan2(np.linalg.norm(product), np.matmul(p1, normals[k, :]).T)
                if angle > np.pi / 2 or angle < -np.pi / 2:
                    normals_sampled[k, :] = - normals_sampled[k, :]
            
            normals_sampled_rotate = np.matmul(normals_sampled, coeff)
            # print(normals_sampled_rotate.shape)
            # fig = plt.figure()
            # ax = fig.add_subplot(211, projection='3d')
            # # print(np.unique(normals_sampled_rotate).shape)
            # ax.scatter(normals_sampled_rotate[:, 0], normals_sampled_rotate[:, 1], normals_sampled_rotate[:, 2])
            # ax.set_xlabel('X Label')
            # ax.set_ylabel('Y Label')
            # ax.set_zlabel('Z Label')
            # # ax = fig.add_subplot(212)
            ...
            # ax = fig.add_subplot(211, projection='3d')
            # # print(np.unique(normals_sampled_rotate).shape)
            # ax.scatter(hand_points_normalized_sampled[:, 0], hand_points_normalized_sampled[:, 1], hand_points_normalized_sampled[:, 2])
            # ax.set_xlabel('X Label')
            # ax.set_ylabel('Y Label')
            # ax.set_zlabel('Z Label')
            # # ax = fig.add_subplot(212)
